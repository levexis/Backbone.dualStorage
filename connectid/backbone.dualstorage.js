...
(function() {
    ...
    var _keys = {};
// TODO: NEEDS TO BE SMART ABOUT POSTS FIRST AND ADD PUTS BACK TO QUEUE
...
// ARE QUEUED.   
    Backbone.Collection.prototype.syncDirty = function() {
        ...
        for (_i = 0, _len = ids.length; _i < _len; _i++) {
            ...
            // most mysterious is that supposed to be a regex
            ...
            if ( isClientKey(  id  ) ) {
                ...
                if ( _keys[id] === 'undefined' ) {
                    ...
                    model.clientId = id;
                    // this creates a stub which may get refrenced by later requests if fired synchronous
...
//    console.log ('store', options.storeName, store );
    ...
    dualsync = function(method, model, options) {
        ...
        // if not dual syncing single sync
// this needs properly reviewing       
        options.dualSync = options.dualSync || model.dualSync;
        // surely this is just determined by the local and remote flags in any case
        var dualSyncNow = !!(model.remote && model.local ||
            !model.dualSync && model.collection && model.collection.remote && model.collection.local); // if collection passed in dualSync wont be there

        console.log('dualSync', dualSyncNow, model, options );
...
//      console.log( 'single sync' , options.remote , options.dualSync , model.dualSync , model );
            if ( result(model, 'remote') || result(model.collection, 'remote') ) {
                ...
                // sets the dirty flag on any changes made in local mode
                if ( result(model, 'local') || result(model.collection, 'local')  ) {
                    // i suppose to comply we should do this if dualSync && ! remote && local
            ...
            // in dual sync mode, not sure why ignoreCallbacks is set to true
                ...
                case 'read':
                    if (localsync('hasDirtyOrDestroyed', model, options)) {
                        ...
                        options.error = function(resp) {
                            // will returns local copy, is this regardless of error code or is existing handler chained? IE what if 403 error?
                            return success(localsync(method, model, options));
                        };
                        return onlineSync(method, model, options);
                    }
